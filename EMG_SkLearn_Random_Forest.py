...
print("Random Forest, Flattened Data")
...
print(f"Test accuracy: {accuracy_score(y_test, rfc.predict(X_test)):.1%}")
...
print("Random Forest, Crafted Features")

# Instantiate the classifier with n_estimators = 10 (default)
cfc = RandomForestClassifier(random_state = 0)

# Fit the model to the training set
cfc.fit(X_train, y_train)

# Testing the model
y_pred = cfc.predict(X_test)

print(f"Test accuracy: {accuracy_score(y_test, cfc.predict(X_test)):.1%}")
